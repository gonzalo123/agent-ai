...
def get_llm(
    ...
    callback_manager = CallbackManager([StreamingStdOutCallbackHandler()]) if DEBUG else CallbackManager(
        [SilentStreamingCallbackHandler()])

    model_kwargs = {
        "max_tokens": max_tokens,
        "stop_sequences": [stop_sequences] if isinstance(stop_sequences, str) else stop_sequences,
